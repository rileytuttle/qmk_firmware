...
static bool sdep_send_pkt(const struct sdep_msg *msg, uint16_t timeout) {
    ...
    do {
        ready = i2c_transmit(I2C_ADDRESS, &msg->type, sizeof(msg->type), SdepTimeout) == I2C_SUCCESS;
        if (ready) {
            break;
        }
    ...
    if (ready) {
        ...
        i2c_transmit(I2C_ADDRESS, &msg->cmd_low, sizeof(*msg) - (1 + sizeof(msg->payload)) + msg->len, SdepTimeout);
        success = true;
    }
...
static bool sdep_recv_pkt(struct sdep_msg *msg, uint16_t timeout) {
    ...
    if (ready) {
        ...
        do {
            ...
            i2c_receive(I2C_ADDRESS, &msg->type, sizeof(msg->type), SdepTimeout);
            ...
            i2c_receive(I2C_ADDRESS, &msg->cmd_low, sizeof(*msg) - (1 + sizeof(msg->payload)), SdepTimeout);

            // and get the payload if there is any
            if (msg->len <= SdepMaxPayload) {
                i2c_receive(I2C_ADDRESS, msg->payload, msg->len, SdepTimeout)
            }
            success = true;
            break;
        } while (timer_elapsed(timerStart) < timeout);
...
static bool ble_init(void) {
    ...
    pinMode(SparkfunBleResetPin, PinDirectionOutput);
    digitalWrite(SparkfunBleResetPin, PinLevelHigh);
    digitalWrite(SparkfunBleResetPin, PinLevelLow);
    _delay_ms(10);
    digitalWrite(SparkfunBleResetPin, PinLevelHigh);
...
bool sparkfun_ble_is_connected(void) { return state.is_connected; }

bool sparkfun_ble_enable_keyboard(void) {
    char resbuf[128];
    ...
    // at this point I would configure the the sparkfun bluetooth thingy so that it works right
...
void sparkfun_ble_task(void) {
    char resbuf[48];

    if (!state.configured && !sparkfun_ble_enable_keyboard()) {
        return;
    }
    resp_buf_read_one(true);
...
bool sparkfun_ble_send_keys(uint8_t hid_modifier_mask, uint8_t *keys, uint8_t nkeys) {
    struct queue_item item;
    bool              didWait = false;
...
bool sparkfun_ble_send_consumer_key(uint16_t keycode, int hold_duration) {
    struct queue_item item;
...
#ifdef MOUSE_ENABLE
bool sparkfun_ble_send_mouse_move(int8_t x, int8_t y, int8_t scroll, int8_t pan, uint8_t buttons) {
    struct queue_item item;
...
uint32_t sparkfun_ble_read_battery_voltage(void) { return state.vbat; }

bool sparkfun_ble_set_mode_leds(bool on) {
    if (!state.configured) {
        return false;
    }
...
bool sparkfun_ble_set_power_level(int8_t level) {
    char cmd[46];
    if (!state.configured) {
        return false;
    }
